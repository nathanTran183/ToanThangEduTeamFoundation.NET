...
namespace ToanThangEdu.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var userActive = db.AspNetUsers.Where(x => x.UserName == model.UserName).FirstOrDefault();
            if (userActive.Activated == true)
            {
                    ...
                    case SignInStatus.Success:
                        {
                            ...
                            if (role.Any(x => x.ToLower().Contains("admin")) || role.Any(x => x.ToLower().Contains("teacher")))
                            {
            ...
            {
                ModelState.AddModelError("", "Tài khoản chưa được xác nhận bởi admin.");
                ViewData["item"] = db.Teachers.OrderByDescending(x => x.Id).ToList();
                return View(model);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var addToRole = await UserManager.AddToRolesAsync(user.Id, "Student");
        ...
        [HttpPost]
        public ActionResult UpdateInfoStudent(ApplicationUser userRedirect, string FullName, string Email, string PhoneNumber, int Tuition, string Address, string TeacherLevel, string StudyContent, int NumberOfSession, string MoreContent)
        {
            //if (ModelState.IsValid)
            //{              
            ...
            //        //var currentUser = (AspNetUser)db.AspNetUsers.FirstOrDefault(x => x.Email == User_Email);
            //        var registerStudentDB = new RegisterStudier();
            //        //TryUpdateModel(registerTeacherDB);
            //        registerStudentDB.Id = userRedirect.Id;
